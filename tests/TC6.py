...
class TestFindBridgeWordsTC6(unittest.TestCase):
    def setUp(self):
        text = (
    ...
    def test_tc6_word2_not_in_graph(self):
        bridge_words, err = self.wg.find_bridge_words("the", "ghost")

        self.assertIsNone(bridge_words)
        self.assertEqual(err, "No the or ghost in the graph!")
