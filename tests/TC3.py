...
class TestFindBridgeWordsTC3(unittest.TestCase):
    def setUp(self):
        text = (
    ...
    def test_tc3_no_bridge_words(self):
        bridge_words, err = self.wg.find_bridge_words("the", "again")

        self.assertEqual(bridge_words, [])
        self.assertEqual(err, "No bridge words from the to again!")
