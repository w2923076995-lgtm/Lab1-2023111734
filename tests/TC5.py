...
class TestFindBridgeWordsTC5(unittest.TestCase):
    def setUp(self):
        text = (
    ...
    def test_tc5_word1_not_in_graph(self):
        bridge_words, err = self.wg.find_bridge_words("ghost", "the")

        self.assertIsNone(bridge_words)
        self.assertEqual(err, "No ghost or the in the graph!")
